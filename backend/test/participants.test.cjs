...
test('CU04 gestiona participantes e invitaciones por HTTP en esquema aislado', async t => {
 ...
 try {
  ...
  const owner = await createUser('ANFITRION'); const otherHost = await createUser('ANFITRION'); const collaborator = await createUser('COLABORADOR'); const second = await createUser('COLABORADOR'); const concurrent = await createUser('COLABORADOR');
  ...
  let invitation; let secondInvitation;
