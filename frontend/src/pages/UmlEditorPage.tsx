...
export function UmlEditorPage() {
  ...
  return <section className="flex h-[calc(100vh-81px)] min-h-[620px] flex-col bg-slate-100">
    <header className="flex flex-wrap items-center justify-between gap-3 border-b bg-white px-4 py-3"><div><Link className="text-sm text-blue-700 underline" to={returnTo}>← Volver</Link><h1 className="text-xl font-bold">Editor UML · {diagram.name}</h1></div><div className="flex flex-wrap items-center gap-2" aria-label="Usuarios conectados">{presence.map(person => <span key={person.id} className="rounded-full bg-emerald-50 px-2 py-1 text-xs text-emerald-800">{person.name} · {person.role}</span>)}<span className={`text-xs ${realtimeConnected ? 'text-emerald-700' : 'text-red-700'}`}>Realtime: {realtimeConnected ? (realtimeReady ? 'conectado' : 'sincronizando') : 'desconectado'}</span><span className="text-sm text-slate-600">{saving || xmiPending ? 'Guardando…' : 'Cambios persistidos'}</span><button disabled={xmiPending} className="rounded border px-3 py-2 disabled:opacity-50" onClick={() => void exportXmi()}>Exportar XMI</button><label className="cursor-pointer rounded border px-3 py-2">Importar XMI<input aria-label="Seleccionar archivo XMI" className="sr-only" type="file" accept=".xmi,.xml,application/xml,text/xml" disabled={xmiPending} onChange={event => { const file = event.currentTarget.files?.[0]; event.currentTarget.value = ''; if (file) void importXmi(file) }}/></label><Link className="rounded border border-violet-700 px-4 py-2 text-violet-700" to={`/projects/${id}/ai-proposal`}>Generar con IA</Link><button disabled={saving || xmiPending || !realtimeReady} className="rounded bg-blue-700 px-4 py-2 text-white disabled:opacity-50" onClick={() => void createClass()}>Nueva clase</button></div></header>
