...
export function UmlEditorPage() {
  ...
  const [generationPending, setGenerationPending] = useState(false); const [generation, setGeneration] = useState<GenerationResult | null>(null)
  ...
  const exportXmi = async () => { setXmiPending(true); setError(null); try { const blob = await xmiService.export(id); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `${diagram?.name ?? 'arqnova-diagram'}.xmi`; link.click(); URL.revokeObjectURL(url); setMessage('Archivo XMI exportado.') } catch (failure) { setError(messageFor(failure)) } finally { setXmiPending(false) } }
  const importXmi = async (file: File) => { if (!confirm('La importación reemplazará el contenido actual del diagrama. ¿Continuar?')) return; setXmiPending(true); setError(null); setMessage(null); try { const result = await xmiService.import(id, file); apply(result.diagram); setSelectedClassId(null); setSelectedRelationId(null); setMessage(`XMI importado: ${result.imported.classes} clases y ${result.imported.relations} relaciones.`) } catch (failure) { setError(messageFor(failure)) } finally { setXmiPending(false) } }
  const generateBackend = async () => { setGenerationPending(true); setError(null); setMessage(null); try { const result = await codeGeneratorService.generate(id); setGeneration(result); setMessage(`Backend generado: ${result.classCount} clases y ${result.fileCount} archivos.`) } catch (failure) { setError(messageFor(failure)) } finally { setGenerationPending(false) } }
  const downloadBackend = async () => { setGenerationPending(true); setError(null); try { const blob = await codeGeneratorService.download(id); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'generated-backend.zip'; link.click(); URL.revokeObjectURL(url); setMessage('Backend Spring Boot descargado.') } catch (failure) { setError(messageFor(failure)) } finally { setGenerationPending(false) } }
  ...
  return <section className="flex h-[calc(100vh-81px)] min-h-[620px] flex-col bg-slate-100">
    <header className="flex flex-wrap items-center justify-between gap-3 border-b bg-white px-4 py-3"><div><Link className="text-sm text-blue-700 underline" to={returnTo}>← Volver</Link><h1 className="text-xl font-bold">Editor UML · {diagram.name}</h1></div><div className="flex flex-wrap items-center gap-2" aria-label="Usuarios conectados">{presence.map(person => <span key={person.id} className="rounded-full bg-emerald-50 px-2 py-1 text-xs text-emerald-800">{person.name} · {person.role}</span>)}<span className={`text-xs ${realtimeConnected ? 'text-emerald-700' : 'text-red-700'}`}>Realtime: {realtimeConnected ? (realtimeReady ? 'conectado' : 'sincronizando') : 'desconectado'}</span><span className="text-sm text-slate-600">{saving || xmiPending || generationPending ? 'Procesando…' : 'Cambios persistidos'}</span><button disabled={xmiPending || generationPending} className="rounded border px-3 py-2 disabled:opacity-50" onClick={() => void exportXmi()}>Exportar XMI</button><label className="cursor-pointer rounded border px-3 py-2">Importar XMI<input aria-label="Seleccionar archivo XMI" className="sr-only" type="file" accept=".xmi,.xml,application/xml,text/xml" disabled={xmiPending || generationPending} onChange={event => { const file = event.currentTarget.files?.[0]; event.currentTarget.value = ''; if (file) void importXmi(file) }}/></label><button disabled={generationPending || xmiPending} className="rounded bg-slate-800 px-3 py-2 text-white disabled:opacity-50" onClick={() => void generateBackend()}>{generationPending ? 'Generando…' : 'Generar Backend'}</button>{generation && <button disabled={generationPending} className="rounded bg-emerald-700 px-3 py-2 text-white disabled:opacity-50" onClick={() => void downloadBackend()}>Descargar ZIP</button>}<Link className="rounded border border-violet-700 px-4 py-2 text-violet-700" to={`/projects/${id}/ai-proposal`}>Generar con IA</Link><button disabled={saving || xmiPending || generationPending || !realtimeReady} className="rounded bg-blue-700 px-4 py-2 text-white disabled:opacity-50" onClick={() => void createClass()}>Nueva clase</button></div></header>
